...
void    Server::recv_request(epoll_event &max_events, int &i, int &epoll_fd, epoll_event &evens_epoll)
{
	int of;
	char buffer[1024];
	std::map<int , Request>::iterator it_request;

	of = recv(max_events.data.fd, buffer, 1023, 0);
	it_request = this->clients_requests.lower_bound(max_events.data.fd);
	if (of == -1)
	{
		this->clients_requests.erase(it_request);
		if (epoll_ctl(epoll_fd, EPOLL_CTL_DEL, max_events.data.fd, NULL) == -1)
			throw std::runtime_error("epoll ctl fialed");
		close (max_events.data.fd);
		return;
	}
	buffer[of] = '\0';
	...
	{ 
		...
		evens_epoll.data.fd = max_events.data.fd;
		if (epoll_ctl(epoll_fd, EPOLL_CTL_MOD, max_events.data.fd, &evens_epoll) == -1)
			throw std::runtime_error("epoll ctl fialed");
	}
}
		...
		for (int i = 0; i < num_of_evens; i++)
		{
			...
			else
			{
				if (max_events[i].events & EPOLLIN)
					this->recv_request(max_events[i], i, epoll_fd, evens_epoll);
				else if (max_events[i].events & EPOLLOUT)
				{
					...
					if (it_response->second.req_autoindex.empty() && it_response->second.return_to.empty())
						ofs = read(it_response->second.fd_body_response, buf, 1023);
					if (ofs > 0 || !it_response->second.req_autoindex.empty() || !it_response->second.return_to.empty())
					{
						...
						if (!it_response->second.header_send)
						{
							...
							if (it_response->second.return_to.empty())
								response << it_response->second.content_type << "\r\n";
							if (!it_response->second.return_to.empty())
								response << "Location: " << it_response->second.return_to << "\r\n";
							else if (!it_response->second.req_autoindex.empty())
								response << "Content-Length: " << it_response->second.req_autoindex.length() << "\r\n";
							else
								response << "Content-Length: " << fileinfo.st_size << "\r\n";
							response << "\r\n";
							it_response->second.header_send = true;
							std::string res = response.str();
							if (!it_response->second.req_autoindex.empty())
								res += it_response->second.req_autoindex;
							...
							{
								this->clients_requests.erase(it_request);
								this->clients_responses.erase(it_response);
						...
						if (it_response->second.req_autoindex.empty() && it_response->second.return_to.empty())
						{
							send(max_events[i].data.fd, buf, ofs, 0);
							ofs = -1;
						}
					}
					else
